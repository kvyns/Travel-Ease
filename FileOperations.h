...
void outputGenerator(string filename, stack<Location*> cities, stack<Route*> routes, bool costOrTime){
    ...
    while(!cities.empty() && !routes.empty()){
        ...
        cout << origin->capital << ", " << origin->country << " --> " << destination->capital << ", " << destination->country;
        cout << " (" << route->transport << " - " << route->time << " hours - $" << cost << ")" << endl;
    ...
    cout << "Total Duration: " << totalTime << " hours" << endl;
